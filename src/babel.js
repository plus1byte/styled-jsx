...
import {
  ...
  getScope,
  processCss,
  createVueComponentImportDeclaration,
  setStateOptions
} from './_utils'
    ...
    visitor: {
      Program: {
        enter(path, state) {
          ...
          if (state.hasJSXStyle || state.file.hasJSXStyle) {
            createVueComponentImportDeclaration(state)
          }
        },
        exit({ scope }, state) {
          if (
          ...
          // const pattern = /_resolveComponent\(\"_JSXStyle\"\)/
          // if (pattern.test(state.code)) {
          //   state.code = state.code.replace(
          //     /_resolveComponent\(\"_JSXStyle\"\)/g,
          //     '_JSXStyle'
          //   )
          // }
          // if (/\.[jt]sx$/.test(state.opts.filename)) {
