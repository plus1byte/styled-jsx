...
export const addClassName = (path, jsxId) => {
  ...
  for (let i = attributes.length - 1, attr; (attr = attributes[i]); i--) {
    ...
    if (t.isJSXSpreadAttribute(attr)) {
      if (t.isObjectExpression(node.argument)) {
        ...
        const index = properties.findIndex(
          property => property.key.name === 'class'
        )
    ...
    if (t.isJSXAttribute(attr) && node.name.name === 'class') {
  ...
  path.node.attributes.push(
    t.jSXAttribute(t.jSXIdentifier('class'), className)
  )
}
...
export const createVueComponentImportDeclaration = state => {
  addDefault(
    state.file.path,
    typeof state.opts.styleModule === 'string'
      ? state.opts.styleModule
      : 'veno-styled-jsx/style',
